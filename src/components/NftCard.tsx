...
export const NFTCard: React.FC<Props> = ({
  ...
  const isSelectedToCart = items.some((item) => item && item.id === nft?.id);
  const isLiked = favouriteItems.some((item) => item && item.id === nft?.id);

  const handleAddToCart = () => {
    dispatch(addItem(nft));

    if (isSelectedToCart) {
      dispatch(removeItem(nft));
    }
  };

  const handleAddToFavorites = () => {
    dispatch(addFavorite(nft));

    if (isLiked) {
      dispatch(removeFavorite(nft));
    }
  };
  ...
  return (
    <div className="shop__card">
      ...
      <div className="shop__card__container">
        ...
        <div className="shop__card-title shop__card-title--secondary">{`Author: ${nft.author}`}</div>
